...
export const RuleHeaders = ({ headerName, headerValue, ruleID, headerIdx }: RuleHeaderProps) => {
  ...
  return <div className={'d-flex w-100 mt-2'}>
    <input className={'me-2 form-control '} value={headerName} onChange={e => dispatch(updateHeaderAction(
      e.target.value,
      headerValue,
      ruleID,
      headerIdx
    ...
    <input className={'form-control'} value={headerValue} onChange={e => {
      dispatch(updateHeaderAction(headerName, e.target.value, ruleID, headerIdx));
    }}/>
    <button className={'btn btn-danger btn-sm ms-2'} onClick={() => {
      dispatch(removeHeaderAction(ruleID, headerIdx));
    }}>
